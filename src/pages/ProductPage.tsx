...
import { ArrowLeft, Star, Check, Shield, Truck, Phone, Mail, ChevronLeft, ChevronRight, ShoppingCart, Send, Share2, User, Building, MessageSquare } from 'lucide-react';
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="space-y-6">
              ...
              <div className="border border-slate-200 rounded-xl p-6">
                <div className="space-y-3">
                  <button className="w-full bg-[#F4B426] hover:bg-[#e6a625] text-white py-4 px-6 rounded-lg font-semibold flex items-center justify-center transition-colors">
                  ...
                  <div className="flex gap-3">
                    <button
                      ...
                      className="flex-1 border-2 border-slate-300 hover:border-[#F4B426] text-slate-700 hover:text-[#F4B426] py-3 px-2 md:px-4 rounded-lg font-semibold transition-colors flex items-center justify-center whitespace-nowrap text-sm md:text-base"
                    >
          ...
          <div className="mt-16">
            ...
            <div className="py-8">
              {activeTab === 'features' && (
                <div>
                  {/* Feature Images Section - Enhanced for N14A and other products with featureImages */}
                  {product.featureImages && product.featureImages.length > 0 ? (
                    <div>
                      {/* Feature Images - Full Size Without Text */}
                      <div>
                        {product.featureImages.map((featureImage, index) => (
                      ...
                      <div className="bg-slate-50 rounded-2xl p-8 mt-8">
                ...
                <div className="grid md:grid-cols-2 gap-8">
                  {Object.entries(product.specifications).map(([key, value]) => (
                    <div key={key} className="border-b border-slate-100 pb-4">
                      <dt className="font-medium text-slate-900 mb-2">{key}</dt>
                      <dd className="text-slate-600">
                        {Array.isArray(value) ? (
                          <ul className="space-y-1">
                            {value.map((item, index) => (
                              <li key={index} className="flex items-center">
                                <span className="w-2 h-2 bg-[#F4B426] rounded-full mr-2"></span>
                                {item}
                              </li>
                            ))}
                          </ul>
              ...
              {activeTab === 'certifications' && (
                <div className="grid grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 md:gap-6">
                  {product.certifications.map((cert, index) => (
                    <div key={index} className="bg-slate-50 rounded-lg p-4 md:p-6 text-center">
                      <Shield className="w-8 h-8 md:w-12 md:h-12 text-[#F4B426] mx-auto mb-2 md:mb-4" />
                      <h4 className="font-semibold text-slate-900 text-sm md:text-base">{cert}</h4>
                    </div>
                  ))}
                </div>
              )}
              ...
              <form onSubmit={handleEnquirySubmit} className="p-6 space-y-6">
                ...
                <div className="flex gap-4">
                  <button
                    type="button"
                    onClick={() => setShowEnquiry(false)}
                    className="flex-1 border-2 border-slate-300 hover:border-slate-400 text-slate-700 py-3 px-4 md:px-6 rounded-lg font-semibold transition-colors text-sm md:text-base"
                  >
                    Cancel
                  </button>
                  <button
                    type="submit"
                    className="flex-1 bg-[#F4B426] hover:bg-[#e6a625] text-white py-3 px-4 md:px-6 rounded-lg font-semibold transition-colors flex items-center justify-center text-sm md:text-base whitespace-nowrap"
                  >
                    <Send className="w-4 h-4 mr-2" />
                    Send Enquiry
                  </button>
